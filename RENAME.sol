...
contract NFTA is ERC721A, Ownable {
    ...
    uint8 public nftPerAddressLimitFreeOG =2; //Max Free mint for WL
    uint8 public nftPerAddressLimitOG =5; //Max Limit per Wallet for WL
        ...
        uint8 isWhitelist
         ...
         if (_mintAmount <= freeMintAvailable) {
             ...
             if(isWhitelist==1){
                ...
                whitelistOGFreeMinted=whitelistOGFreeMinted+_mintAmount;
             }
            ...
            if(isWhitelist==1){
                whitelistFreeMinted=whitelistFreeMinted+freeMintAvailable;
                 whitelistOGFreeMinted=whitelistOGFreeMinted+freeMintAvailable;
            }
            else if(isWhitelist==2){
                whitelistFreeMinted=whitelistFreeMinted+freeMintAvailable;
        ...
        returns (uint256)
    {
        ...
        if (maxFreeMintAmount > currentMintAmount) {
            ...
            if((isWhitelist==1&&whitelistOGFreeMinted<whitelistOGLimit)){
                
                if(freeMintAvailable<whitelistOGLimit-whitelistOGFreeMinted){
                    return freeMintAvailable;
                }
                else{
                    return(whitelistOGLimit-whitelistOGFreeMinted);
                }
    ...
    function preSaleMintOG(uint8 _mintAmount) preConditions(_mintAmount) public payable {
        ...
        require(_mintAmount+ownerTokenCount<= nftPerAddressLimitOG,"Max lmit of tokens exceeded for Whitelists");
        ...
        if (isWhiteListedO(msg.sender)) {
            require(ownerTokenCount < nftPerAddressLimitOG,"Max lmit of tokens exceeded for OG list");
            uint256 freeMintAvailable = getFreeMintAvailableAmount(
                nftPerAddressLimitFreeOG,1
            );
    ...
    ///@param _nftPerAddressLimitFreeOG of type uint8 is the number of free tokens allowed to mint in OG whitelist
    function setnftPerAddressLimitFreeOG (uint8 _nftPerAddressLimitFreeOG) public onlyOwner{
       nftPerAddressLimitFreeOG=_nftPerAddressLimitFreeOG;
    ...
    function setnftPerAddressLimitOG (uint8 _nftPerAddressLimitOG) public onlyOwner{
        nftPerAddressLimitOG=_nftPerAddressLimitOG;
    }
